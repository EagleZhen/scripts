...
def read_pbf_lines(pbf_file):
	with open(pbf_file, 'rb') as file:
		raw = file.read()

	# Real PotPlayer bookmarks are UTF-16 (with BOM); ones generated by
	# `infowriter to pbf.py` are plain UTF-8. Detect via BOM, since there's
...
def convert_pbf_to_youtube_timestamps(pbf_file):
	lines = read_pbf_lines(pbf_file)[1:]  # Skip the first line
